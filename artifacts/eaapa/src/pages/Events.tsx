  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
          <h1 className="text-4xl md:text-5xl font-display font-bold text-foreground mb-4">Ecosystem Events</h1>
          <p className="text-xl text-muted-foreground">Conferences, capacity building, and networking across the region.</p>
        </div>
        
        <div className="flex items-center gap-4 bg-card border border-border p-1.5 rounded-2xl">
          <button onClick={()=>setView("list")} className={clsx("flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm transition-all", view==="list" ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground")}>
            <LayoutList className="w-4 h-4"/> List
          </button>
          <button onClick={()=>setView("calendar")} className={clsx("flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm transition-all", view==="calendar" ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground")}>
      ...
      <div className="flex gap-2 mb-8 overflow-x-auto no-scrollbar pb-2">
        {["All", "Conference", "Exhibition", "Workshop", "Webinar", "Training"].map(t => (
          <button 
            ...
            className={clsx("px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all", typeFilter===t ? "bg-primary text-white" : "bg-card border border-border text-muted-foreground hover:bg-muted")}
          >
            {t}
          </button>
        ))}
      ...
      {view === "list" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {filteredEvents.map((evt) => (
            <div key={evt.id} className={clsx(
              ...
              evt.isFeatured ? "border-secondary/50 shadow-[0_0_30px_rgba(217,119,6,0.1)]" : "border-border"
            )}>
              ...
              <div className="p-8 flex flex-col h-full relative">
                ...
                <span className="px-3 py-1 rounded bg-muted/50 text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-4 inline-table self-start">
                  {evt.type}
                </span>
                
                <h3 className="text-xl font-bold text-foreground mb-3 group-hover:text-primary transition-colors leading-tight">{evt.title}</h3>
                <p className="text-sm text-muted-foreground mb-8 flex-1">{evt.description}</p>
                
                <div className="space-y-4 mb-8 bg-background/50 p-5 rounded-2xl border border-border">
                ...
                <button 
                  ...
                  className={clsx(
                    ...
                    evt.isRegistered ? "bg-muted text-foreground border border-white/20" :
        ...
        <div className="glass-panel rounded-3xl p-8 border-border">
          <h2 className="text-2xl font-bold text-foreground mb-6">{format(today, 'MMMM yyyy')}</h2>
            ...
            {daysInMonth.map((day, i) => {
              ...
              return (
                <div key={i} className={clsx(
                  ...
                  isToday(day) ? "bg-primary/10 border-primary" : "bg-background/50 border-border hover:border-white/20",
                  !isSameMonth(day, today) && "opacity-50"
                )}>
                  <div className={clsx("text-sm font-bold mb-2", isToday(day) ? "text-primary" : "text-foreground/65")}>{format(day, 'd')}</div>
                  <div className="space-y-1.5">
                    {dayEvents.map(e => (
                      <div key={e.id} className="text-[10px] p-1.5 rounded bg-card border border-border text-white font-medium truncate" title={e.title}>
                        {e.title}
                      </div>
                    ))}
                  </div>
